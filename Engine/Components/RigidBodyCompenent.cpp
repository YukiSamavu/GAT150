...
namespace nc
{
	...
	{
		m_owner->m_enigne->GetSystem<PhysicsSystem>()->DestroyBody(m_body);
	}
	...
	void RigidBodyComponent::Read(const rapidjson::Value& value)
	{
		...
		json::Get(value, "gravityScae", m_data.gravityScale);
	}
	...
	void RigidBodyComponent::Update()
	{
		if (m_body == nullptr)
		{
			m_body = m_owner->m_enigne->GetSystem<PhysicsSystem>()->CreateBody(m_owner->m_transform.position, m_owner->m_transform.angle, m_data, m_owner);
